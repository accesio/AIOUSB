...
int main( int argc, char **argv ) {
	printf(
		"USB-DIO-16A TRANSMIT sample program version 0.1\n"
		"  AIOUSB library version %s, %s\n"
		"  This program demonstrates high speed streaming data output on\nUSB-DIO-16H Family devices\n"
		"  For simplicity, it uses the first AIOUSB device found on the bus.\n"
		"  /TX_EN MUST BE LOW\n"
	...
	if( result == AIOUSB_SUCCESS ) {
		...
		if( deviceMask != 0 ) {
			...
			uint64_t serialNumber1 = 0;		// serial number of board
			int deviceIndex1 = -1
				, index = 0;
			while( deviceMask != 0 ) {
				if( ( deviceMask & 1 ) != 0 ) {
			...
			if(
				...
				if( frameData != 0 ) {
					 ...
					 */
					{
						 ...
						 * configure sending device to send streaming DIO data;
						 */
